...
import { Upload, Download, RefreshCw, AlertCircle, Image as ImageIcon, Crop, FileArchive, Scissors, FlipHorizontal, RotateCw, Moon } from "lucide-react";
  ...
  return (
    <>
      <SEO
        ...
        description="Compress, convert, resize, flip, rotate, and grayscale images online for free. Support for JPG, PNG, WebP."
        keywords="image compressor, convert png to jpg, resize image online, flip image, rotate image, grayscale image, profile picture maker"
        url="/image-tools"
      />
      <div className="max-w-4xl mx-auto space-y-8">
        <div className="text-center space-y-2">
          ...
          <p className="text-slate-600">Compress, resize, convert, flip, rotate and more.</p>
        </div>
            ...
            {!file ? (
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-200">
                  ...
                  <button onClick={() => setFile(null)} className="text-slate-400 hover:text-red-500 transition-colors">Clear</button>
                </div>
                ...
                {activeTab === "convert" && (
                  <div className="space-y-4 bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
                    ...
                    <select value={targetFormat} onChange={(e) => setTargetFormat(e.target.value)} className="w-full p-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500">
                ...
                {activeTab === "resize" && (
                  <div className="space-y-4 bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
                    <p className="text-sm text-slate-500">Leave one field empty to maintain aspect ratio.</p>
                ...
                {activeTab === "profile" && (
                  <div className="space-y-4 bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
                    <p className="text-sm text-slate-500">Image will be cropped to a circle with transparent background.</p>
                    <div className="flex justify-between">
                ...
                {activeTab === "grayscale" && (
                  <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
                    <p className="text-slate-600">Converts your image to black & white. Output will be a PNG.</p>
                  </div>
                )}

                {activeTab === "flip" && (
                  <div className="space-y-4 bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
                    <label className="font-semibold text-slate-700 block">Flip Direction</label>
                    <div className="grid grid-cols-2 gap-3">
                      {["horizontal", "vertical"].map((d) => (
                        <button
                          key={d}
                          onClick={() => setFlipDirection(d)}
                          className={`py-3 rounded-xl font-semibold border-2 transition-colors capitalize ${flipDirection === d ? "border-blue-500 bg-blue-50 text-blue-700" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}
                        >
                          {d}
                        </button>
                      ))}
                ...
                {activeTab === "rotate" && (
                  <div className="space-y-4 bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
                    <label className="font-semibold text-slate-700 block">Rotation Angle</label>
                    <div className="grid grid-cols-3 gap-3">
                      {["90", "180", "270"].map((a) => (
                        <button
                          key={a}
                          onClick={() => setRotateAngle(a)}
                          className={`py-3 rounded-xl font-semibold border-2 transition-colors ${rotateAngle === a ? "border-blue-500 bg-blue-50 text-blue-700" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}
                        >
                          {a}°
                        </button>
                      ))}
                ...
                {!downloadUrl ? (
                  ...
                  <a href={downloadUrl} download className="w-full py-4 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl font-bold text-lg shadow-lg flex justify-center items-center gap-2">
