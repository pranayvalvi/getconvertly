...
export default function PDFTools() {
  ...
  const processApi = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/pdf${endpoint}`, {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <>
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-slate-600">Securely merge, split, watermark, and convert objects to PDF.</p>
      </div>
        ...
        <div className="flex border-b border-slate-200 bg-white/50 overflow-x-auto">
          {tabs.map(tab => (
            <button
              ...
              className={`flex-1 min-w-[140px] py-4 flex items-center justify-center gap-2 font-semibold transition-colors
            ...
            <div className="space-y-6">
              ...
              {activeTab === "split" && (
                <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm space-y-2">
                  ...
                  <p className="text-xs text-slate-500">Leave blank to split all pages individually.</p>
                </div>
              )}
              ...
              {!downloadUrl ? (
                ...
                <a 
                  href={downloadUrl}
                  download
                  className="w-full py-4 bg-brand-500 hover:bg-brand-600 text-white rounded-xl font-bold text-lg shadow-lg flex justify-center gap-2"
                >
